            ...
            for (size_t i = 0; i < rule->get_conditions().size(); ++i) {
                ...
                if (project_away) {
                    ...
                    std::unique_ptr<RuleBase> new_rule = std::make_unique<ProjectRule>(0,
                                                                                new_atom,
                                                                                std::vector<DatalogLiteral>{condition},
                                                                                nullptr);
...
Arguments Datalog::get_relevant_joining_arguments_from_component(const DatalogAtom &rule_head, const std::vector<DatalogLiteral> &conditions) {
